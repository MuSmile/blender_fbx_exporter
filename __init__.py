...
if "bpy" in locals():
	...
	imp.reload(op_pivot_center)
	imp.reload(op_select_all)
	...
	from . import op_pivot_center
	from . import op_select_all
...
class FBXBundleSettings(bpy.types.PropertyGroup):
	...
	mode_bundle: bpy.props.EnumProperty(items= 
		...
		('COLLECTION', 'Collection', "Bundle by objects collection with their name rules"),
		('ROOT_COLLECTION', 'Root Collection', "Bundle by objects root collection with their name rules"),
	...
	mode_pivot: bpy.props.EnumProperty(items=[
		...
		('BOUNDS_CENTER', 'Bounds Center', "Pivot at the center of the bounds of the bundle"), 
...
class Panel_Tools(bpy.types.Panel):
	...
	def draw(self, context):
		...
		col.operator(op_pivot_center.op.bl_idname, text="Pivot at Bound Center", icon='OUTLINER_DATA_EMPTY')
		col.operator(op_select_all.op.bl_idname, text="Select All Mesh", icon='SELECT_SET')
