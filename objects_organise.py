...
def get_root_collection_name(obj):
...
def get_key(obj):
	...
	if mode_bundle == 'NAME':
		...
		matchObj = re.match( '\[(.*)\].*', obj.name)
		if matchObj:
			name = matchObj.group(1)
		return name

	elif mode_bundle == 'ROOT_COLLECTION':
		name = get_root_collection_name(obj)
		matchObj = re.match( '\[(.*)\].*', obj.name)
		if matchObj:
			name = matchObj.group(1)
		return name
